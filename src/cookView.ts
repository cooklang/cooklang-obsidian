...
import {CooklangParser, CooklangRecipe, HTMLRenderer} from "@cooklang/cooklang-ts";
...
export class CookView extends TextFileView {
	...
	getViewData() {
		...
		this.recipe = this.parser.parse(this.data)[0];
		return this.data;
	}
	...
	renderPreview(recipe: CooklangRecipe) {
		...
		this.previewEl.innerHTML = (new HTMLRenderer).render(recipe);
